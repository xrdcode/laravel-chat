                        ...
                        <form id="chatform" action="POST">
                            ...
                            <div style="max-height: 300px;overflow-y: scroll">
                                <table id="chats" class="table table-responsive">
                                    @foreach($product->chat_messages()->where('user_id', Auth::id())->get() as $cm)
                                        <tr>
                                            @if($cm->isFromUser)
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $("#chatform").on("submit", function(e) {
            ...
            console.log($("textarea,input", this).serialize())

            $.ajax({
                url: '{{ route('chat.send',$product->id) }}',
        ...
        Echo.private('product-chat_{{ Auth::id() }}_{{ $product->id }}').listen('ChatSent', function(data) {
        ...
        function fetchMessage(table) {
            $.getJSON('{{ route("chat.fetch", $product->id) }}', function(data) {
                console.log(data)
            });
        }
